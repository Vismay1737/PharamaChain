    ...
    <div className="min-h-screen t-bg flex items-center justify-center p-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold t-text">PharmaChain</h1>
          <p className="text-xs t-text-muted mt-1">AI-Powered Supply Chain Guardian</p>
        </div>

        <div className="card p-7">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-xs font-semibold t-text-secondary mb-1.5 block">Username</label>
              <div className="relative">
                <UserIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 t-text-muted" size={16} />
                <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Enter username"
                  className="w-full t-bg border t-border rounded-lg py-3 pl-10 pr-4 outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-400 text-sm t-text placeholder:t-text-muted" required />
              </div>
            </div>
            <div>
              <label className="text-xs font-semibold t-text-secondary mb-1.5 block">Password</label>
              <div className="relative">
        ...
        <p className="text-center text-[10px] t-text-muted mt-4">Google Solution Challenge 2026 · SDG 3</p>
      </motion.div>
    </div>
  );
};
