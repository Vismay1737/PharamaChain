        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages.length === 1 && (
            <div className="mt-6 mb-4">
              <div className="text-center mb-6">
                <div className="w-16 h-16 mx-auto mb-3 rounded-xl dark:bg-green-500/10 bg-green-50 border dark:border-green-500/20 border-green-100 flex items-center justify-center">
                  <BrainCircuit size={32} className="dark:text-green-400 text-green-500" />
                </div>
                <h2 className="text-lg font-bold t-text">Gemini-Powered Intelligence</h2>
                <p className="text-sm t-text-muted">Ask about batches, risks, or compliance</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 max-w-xl mx-auto">
                {SUGGESTIONS.map((s, i) => (
                  <button key={i} onClick={() => sendMessage(s)} className="card p-3.5 text-left text-sm t-text-secondary hover:border-green-300 hover:text-green-700 transition-all flex items-start gap-2.5">
                    <Sparkles size={14} className="dark:text-green-400 text-green-500 shrink-0 mt-0.5" />{s}
                  </button>
                ))}
              </div>
            </div>
          ...
          <AnimatePresence>
            {messages.map((msg, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-2.5 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                ...
                <div className={`max-w-[70%] p-3.5 rounded-xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-green-500 text-white' : 'card t-text'}`}>
                  {msg.text.split('**').map((part, j) => j % 2 === 1 ? <strong key={j} className={msg.role === 'user' ? 'text-white' : 'dark:text-green-400 text-green-600'}>{part}</strong> : part)}
                </div>
          ...
          {loading && (
            <div className="flex gap-2.5"><div className="w-7 h-7 rounded-lg bg-green-500 flex items-center justify-center"><Bot size={14} className="text-white" /></div>
              <div className="card p-3.5 rounded-xl"><Loader2 size={16} className="dark:text-green-400 text-green-500 animate-spin" /></div></div>
          )}
          <div ref={bottomRef} />
        </div>
        <div className="p-4 border-t t-border t-card">
          <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex gap-2.5 max-w-2xl mx-auto">
            <input value={input} onChange={e => setInput(e.target.value)} placeholder="Ask about your supply chain..." className="flex-1 t-bg border t-border rounded-lg px-4 py-2.5 text-sm t-text placeholder:t-text-muted outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-300" disabled={loading} />
